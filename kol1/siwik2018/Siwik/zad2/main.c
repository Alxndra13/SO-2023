...
int main(int argc, char *argv[])
{
   ...
   if ((pid = fork()) == 0)
   {
      ...
      // close unused read/write ends of pipies
      close(toChildFD[1]);
      close(toParentFD[0]);

      read(toChildFD[0], &val2, sizeof(val2)); //read value passed by the parent from the pipe and save to val2
      close(toChildFD[0]);
   ...
   else
   {
      ...
      close(toChildFD[0]); //close unused
      close(toParentFD[1]);
